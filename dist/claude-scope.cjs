...
var ExternalEditor = class {
  ...
  createTemporaryFile() {
    try {
      const baseDir = this.fileOptions.dir ?? import_node_os2.default.tmpdir();
...
function formatTool(name, target, colors8) {
  const nameStr = colorize(name, colors8.tools.name);
  if (target) {
    const targetStr = colorize(`: ${truncatePath(target)}`, colors8.tools.target);
...
var activeToolsStyles = {
   ...
   */
  balanced: (data, colors8) => {
    const parts = [];
    const c = colors8 ?? getDefaultColors();
  ...
  playful: (data, colors8) => {
    ...
    for (const tool of data.running.slice(-3)) {
      ...
      const nameStr = colors8 ? colorize(tool.name, colors8.tools.name) : tool.name;
  ...
  labeled: (data, colors8) => {
    ...
    const prefix = colors8 ? colorize("Tools:", c.semantic.info) : "Tools:";
    return `${prefix}: ${allTools.join(" | ")}`;
  },
  /**
   * indicator: ● bullet indicators
   */
  indicator: (data, colors8) => {
    const parts = [];
    const c = colors8 ?? getDefaultColors();
    for (const tool of data.running) {
...
function getCacheColor(hitRate, colors8) {
  if (hitRate > 70) {
    return colors8.cache.high;
  } else if (hitRate >= 40) {
    return colors8.cache.medium;
  } else {
    return colors8.cache.low;
  }
}
var cacheMetricsStyles = {
  ...
  balanced: (data, colors8) => {
    ...
    const color = colors8 ? getCacheColor(hitRate, colors8) : "";
  ...
  compact: (data, colors8) => {
    ...
    if (colors8) {
      return `${colors8.cache.read}Cache: ${amount}`;
    }
    return `Cache: ${amount}`;
  },
  /**
  ...
  verbose: (data, colors8) => {
    ...
    const amount = formatK(cacheRead);
    const saved = colors8 ? `${colors8.cache.write}${formatCurrency(savings)} saved` : `${formatCurrency(savings)} saved`;
...
var ConfigProvider = class {
  ...
  async scanConfigs(options) {
    ...
    if (await this.fileExists(path2.join(claudeDir, "CLAUDE.md"))) {
      claudeMdCount++;
    }
    rulesCount += await this.countRulesInDir(path2.join(claudeDir, "rules"));
    const userSettings = path2.join(claudeDir, "settings.json");
    ...
    const userClaudeJson = path2.join(homeDir, ".claude.json");
    ...
    if (cwd) {
      if (await this.fileExists(path2.join(cwd, "CLAUDE.md"))) {
        claudeMdCount++;
      }
      if (await this.fileExists(path2.join(cwd, "CLAUDE.local.md"))) {
        claudeMdCount++;
      }
      if (await this.fileExists(path2.join(cwd, ".claude", "CLAUDE.md"))) {
        claudeMdCount++;
      }
      if (await this.fileExists(path2.join(cwd, ".claude", "CLAUDE.local.md"))) {
        claudeMdCount++;
      }
      rulesCount += await this.countRulesInDir(path2.join(cwd, ".claude", "rules"));
      const mcpJson = path2.join(cwd, ".mcp.json");
      const mcpData = await this.readJsonFile(mcpJson);
      if (mcpData) {
        mcpCount += this.countMcpServers(mcpData);
      }
      const projectSettings = path2.join(cwd, ".claude", "settings.json");
      ...
      const localSettings = path2.join(cwd, ".claude", "settings.local.json");
  ...
  async countRulesInDir(rulesDir) {
    ...
    try {
      ...
      for (const entry of entries) {
        const fullPath = path2.join(rulesDir, entry.name);
...
function getContextColor(percent, colors8) {
  const clampedPercent = Math.max(0, Math.min(100, percent));
  if (clampedPercent < 50) {
    return colors8.low;
  } else if (clampedPercent < 80) {
    return colors8.medium;
  } else {
    return colors8.high;
  }
}
var contextStyles = {
  balanced: (data, colors8) => {
    ...
    if (!colors8) return output;
    return colorize(output, getContextColor(data.percent, colors8));
  },
  compact: (data, colors8) => {
  ...
  playful: (data, colors8) => {
    ...
    if (!colors8) return output;
    return `\u{1F9E0} ${colorize(`[${bar}] ${data.percent}%`, getContextColor(data.percent, colors8))}`;
  },
  verbose: (data, colors8) => {
...
function balancedStyle(data, colors8) {
  const formatted = formatCostUSD(data.costUsd);
  if (!colors8) return formatted;
  const amountStr = data.costUsd.toFixed(2);
  return colorize("$", colors8.currency) + colorize(amountStr, colors8.amount);
}
var costStyles = {
  ...
  playful: (data, colors8) => {
    const formatted = formatCostUSD(data.costUsd);
    if (!colors8) return `\u{1F4B0} ${formatted}`;
    const amountStr = data.costUsd.toFixed(2);
    const colored = colorize("$", colors8.currency) + colorize(amountStr, colors8.amount);
    return `\u{1F4B0} ${colored}`;
  },
  labeled: (data, colors8) => {
    const formatted = formatCostUSD(data.costUsd);
    if (!colors8) return withLabel("Cost", formatted);
    const amountStr = data.costUsd.toFixed(2);
    const colored = colorize("$", colors8.currency) + colorize(amountStr, colors8.amount);
  ...
  indicator: (data, colors8) => {
    const formatted = formatCostUSD(data.costUsd);
    if (!colors8) return withIndicator(formatted);
    const amountStr = data.costUsd.toFixed(2);
    const colored = colorize("$", colors8.currency) + colorize(amountStr, colors8.amount);
...
var durationStyles = {
  balanced: (data, colors8) => {
    ...
    if (!colors8) return formatted;
    return formatDurationWithColors(data.durationMs, colors8);
  },
  compact: (data, colors8) => {
    ...
    if (hours > 0) {
      return colorize(`${hours}`, colors8.value) + colorize("h", colors8.unit) + colorize(`${minutes}`, colors8.value) + colorize("m", colors8.unit);
    }
    return colorize(`${minutes}`, colors8.value) + colorize("m", colors8.unit);
  },
  playful: (data, colors8) => {
    const totalSeconds = Math.floor(data.durationMs / 1e3);
    const hours = Math.floor(totalSeconds / 3600);
    ...
    if (hours > 0) {
      const colored = colorize(`${hours}`, colors8.value) + colorize("h", colors8.unit) + colorize(` ${minutes}`, colors8.value) + colorize("m", colors8.unit);
      return `\u231B ${colored}`;
    }
    return `\u231B ${colorize(`${minutes}`, colors8.value)}${colorize("m", colors8.unit)}`;
  },
  technical: (data, colors8) => {
    const value = `${Math.floor(data.durationMs)}ms`;
    if (!colors8) return value;
    return colorize(`${Math.floor(data.durationMs)}`, colors8.value) + colorize("ms", colors8.unit);
  },
  labeled: (data, colors8) => {
    const formatted = formatDuration(data.durationMs);
    if (!colors8) return withLabel("Time", formatted);
    const colored = formatDurationWithColors(data.durationMs, colors8);
...
function formatDurationWithColors(ms, colors8) {
  if (ms <= 0) return colorize("0s", colors8.value);
  ...
  if (hours > 0) {
    parts.push(
      colorize(`${hours}`, colors8.value) + colorize("h", colors8.unit),
      colorize(`${minutes}`, colors8.value) + colorize("m", colors8.unit),
      colorize(`${seconds}`, colors8.value) + colorize("s", colors8.unit)
    );
  } else if (minutes > 0) {
    parts.push(
      colorize(`${minutes}`, colors8.value) + colorize("m", colors8.unit),
    ...
    parts.push(colorize(`${seconds}`, colors8.value) + colorize("s", colors8.unit));
  }
  return parts.join(" ");
}
...
var gitTagStyles = {
  balanced: (data, colors8) => {
    const tag = data.tag || "\u2014";
    if (!colors8) return tag;
    return colorize(tag, colors8.branch);
  },
  compact: (data, colors8) => {
    if (!data.tag) return "\u2014";
  ...
  playful: (data, colors8) => {
    const tag = data.tag || "\u2014";
    if (!colors8) return `\u{1F3F7}\uFE0F ${tag}`;
    return `\u{1F3F7}\uFE0F ${colorize(tag, colors8.branch)}`;
  },
  verbose: (data, colors8) => {
    if (!data.tag) return "version: none";
    const tag = `version ${data.tag}`;
    if (!colors8) return tag;
    return `version ${colorize(data.tag, colors8.branch)}`;
  },
  labeled: (data, colors8) => {
    const tag = data.tag || "none";
    if (!colors8) return withLabel("Tag", tag);
    return withLabel("Tag", colorize(tag, colors8.branch));
  },
  indicator: (data, colors8) => {
    const tag = data.tag || "\u2014";
    if (!colors8) return withIndicator(tag);
    return withIndicator(colorize(tag, colors8.branch));
  }
};
...
var GitTagWidget = class {
   ...
   */
  constructor(gitFactory, colors8) {
...
var gitStyles = {
  minimal: (data, colors8) => {
    if (!colors8) return data.branch;
    return colorize(data.branch, colors8.branch);
  },
  balanced: (data, colors8) => {
    if (data.changes && data.changes.files > 0) {
      ...
      if (parts.length > 0) {
        const branch = colors8 ? colorize(data.branch, colors8.branch) : data.branch;
        const changes = colors8 ? colorize(`[${parts.join(" ")}]`, colors8.changes) : `[${parts.join(" ")}]`;
        return `${branch} ${changes}`;
      }
    }
    return colors8 ? colorize(data.branch, colors8.branch) : data.branch;
  },
  compact: (data, colors8) => {
    if (data.changes && data.changes.files > 0) {
      const parts = [];
  ...
  playful: (data, colors8) => {
    if (data.changes && data.changes.files > 0) {
      ...
      if (data.changes.deletions > 0) parts.push(`\u2B07${data.changes.deletions}`);
      if (parts.length > 0) {
        const branch2 = colors8 ? colorize(data.branch, colors8.branch) : data.branch;
  ...
  indicator: (data, colors8) => {
    ...
    return withIndicator(colors8 ? colorize(data.branch, colors8.branch) : data.branch);
  }
};
...
var linesStyles = {
  balanced: (data, colors8) => {
    if (!colors8) return `+${data.added}/-${data.removed}`;
    const addedStr = colorize(`+${data.added}`, colors8.added);
    const removedStr = colorize(`-${data.removed}`, colors8.removed);
  ...
  verbose: (data, colors8) => {
    ...
    if (data.added > 0) {
      ...
      parts.push(colors8 ? colorize(text, colors8.added) : text);
    }
    if (data.removed > 0) {
      const text = `-${data.removed} removed`;
      parts.push(colors8 ? colorize(text, colors8.removed) : text);
    }
    return parts.join(", ");
  },
  labeled: (data, colors8) => {
    const addedStr = colors8 ? colorize(`+${data.added}`, colors8.added) : `+${data.added}`;
    const removedStr = colors8 ? colorize(`-${data.removed}`, colors8.removed) : `-${data.removed}`;
...
var modelStyles = {
  balanced: (data, colors8) => {
    if (!colors8) return data.displayName;
    return colorize(data.displayName, colors8.name);
  },
  compact: (data, colors8) => {
    const shortName = getShortName(data.displayName);
    if (!colors8) return shortName;
    return colorize(shortName, colors8.name);
  },
  playful: (data, colors8) => {
    const shortName = getShortName(data.displayName);
    if (!colors8) return `\u{1F916} ${shortName}`;
    return `\u{1F916} ${colorize(shortName, colors8.name)}`;
  },
  technical: (data, colors8) => {
    if (!colors8) return data.id;
    const match = data.id.match(/^(.+?)-(\d[\d.]*)$/);
    if (match) {
      return colorize(match[1], colors8.name) + colorize(`-${match[2]}`, colors8.version);
    }
    return colorize(data.id, colors8.name);
  },
  symbolic: (data, colors8) => {
    const shortName = getShortName(data.displayName);
    if (!colors8) return `\u25C6 ${shortName}`;
  ...
  labeled: (data, colors8) => {
    const shortName = getShortName(data.displayName);
    if (!colors8) return withLabel("Model", shortName);
    return withLabel("Model", colorize(shortName, colors8.name));
  },
  indicator: (data, colors8) => {
    const shortName = getShortName(data.displayName);
    if (!colors8) return withIndicator(shortName);
    return withIndicator(colorize(shortName, colors8.name));
  }
};
...
var Deck = class {
  ...
  shuffle() {
    for (let i = this.cards.length - 1; i > 0; i--) {
      const j = (0, import_node_crypto2.randomInt)(0, i + 1);
...
function formatHandResult(handResult, colors8) {
  if (!handResult) {
    return "\u2014";
  }
  ...
  if (!colors8) return resultText;
  return colorize2(resultText, colors8.result);
}
...
function balancedStyle2(data, colors8) {
  ...
  const labelColor = colors8?.participating ?? lightGray;
  const handLabel = colorize2("Hand:", labelColor);
  const boardLabel = colorize2("Board:", labelColor);
  return `${handLabel} ${handStr}| ${boardLabel} ${boardStr}\u2192 ${formatHandResult(handResult, colors8)}`;
}
var pokerStyles = {
  ...
  "compact-verbose": (data, colors8) => {
    ...
    if (!colors8) return result;
    return colorize2(result, colors8.result);
  },
  emoji: (data, colors8) => {
    const { holeCards, boardCards, handResult } = data;
    const participatingSet = new Set(handResult?.participatingIndices || []);
